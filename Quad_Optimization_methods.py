...
def exhaustive_search(n,step,starting_point):
    lamdas = []
    lamdas.append(starting_point + step)
    lamdas.append(lamdas[0] + step)
    lamdas.append(lamdas[1] + step)
...
def cubic_interpolation(f,df,acc,x1,t_not):
    temp = f(x1)
    ...
    while true:
        ...
        Q = pow((pow(Z,2) - df(x1)*df(x2)),0.5)
        b = (1/pow((x1-x2),2))*(pow(x2,2)*b2 + pow(x1,2)*b4 + 2*Z*x1*x2)#X[1,0]
        c = -(1/(pow((x1-x2),2)))*((x1+x2)*Z + x2*b2 + x1*b4)#X[2,0]
        d = (1/(3*pow((x1-x2),2)))*(2*Z + b2 + b4)#X[3,0]
        keera=(pow(c, 2) - 3 * b * d)
        x_star = (-1*c + pow((pow(c,2)-3*b*d),0.5))/(3*d)#float(x1 + ((-1*a1)/(a2 + sqrt((keera)))))
        check = abs(df(x_star))
        if check < acc:
            break
        else:
...
def quad_interpolation(x1,t_not,acc):
    ...
    while error >= acc:
        ...
        a1 = ((function(x1)) * (pow(x2, 2) - pow(x3, 2)) + function(x2) * (pow(x3, 2) - pow(x1, 2)) + function(x3) * (pow(x1, 2) - pow(x2, 2))) / ((x1 - x2) * (x2 - x3) * (x3 - x1))
        a2 = -1 * (function(x1) * (x2 - x3) + function(x2) * (x3 - x1) + function(x3) * (x1 - x2)) / ((x1 - x2) * (x2 - x3) * (x3 - x1))
        x_star = -1*(a1/(2*a2))## (P_x(a0,a1,a2,x1)*(pow(x2,2) - pow(x3,2)) + P_x(a0,a1,a2,x2)*(pow(x3,2) - pow(x1,2)) + P_x(a0,a1,a2,x3)*(pow(x1,2) - pow(x2,2)))/(2*(P_x(a0,a1,a2,x1)*(x2 - x3) + P_x(a0,a1,a2,x2)*(x3 - x1) + P_x(a0,a1,a2,x3)*(x1 - x2)))
        error = abs((function(x_star) - P_x(a0,a1,a2,x_star))/function(x_star))
...
x = symbols('x')
f = x/log(x)  #pow(x,5) - 5*pow(x,3) - 20*x +5# 0.65- (0.75/(1 + pow(x,2))) - 0.65*x*(atan(pow(x,-1)))
df = diff(f)
d2f = diff(df)
print(df)
f = lambdify(x, f)
...
a,b = golden_section(2,3,0.1,0)#(a,b,accuracy,min or max)
print("Golden section Interval: ",a,b)

a,b = fibonacci_search(2,3,0.05,feb,0)#(a,b,accuracy,febonacci_sequence,min or max)
print("fibonacci Interval: ",a,b)

a,b = dichotomous_search(2,3,0.005,0.001,0)#(a,b,accuracy,delta,min or max)
print("dichotomous Interval: ",a,b)

a,b = interval_halving(2,3,0.001,0)#(a,b,accuracy,min or max)
print("Interval halving Interval: ",a,b)

a= quad_interpolation(2,0.1,0.01)#x1,t_not,acc
print("Quad interpolation Approx sol: ",a)

x_star = cubic_interpolation(f,df,0.1,2,0.1)#f,df,acc,x1,t_not
...
x_star = seacant_method(df,0.001,2,0.1)#df,epsilon,A,t0
...
x_star = exhaustive_search(20,0.1,2)#n,step,starting_point
